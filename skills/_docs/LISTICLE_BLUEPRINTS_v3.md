# LISTICLE_BLUEPRINTS_v3 (Blog List Writer v3.0)

This document defines the **lintable** (machine-checkable) blueprints for `blog-list-writer` `content_type=list`.

Design goal: make listicles **compile-like** — fixed heading order, fixed tables, fixed CTA markers, and fixed tool-card fields — so a validator can **PASS/FAIL** structure before downstream editing/AEO/export.

**Version**: 3.0  
**Updated**: 2026-02-04  
**Used by**: `skills/writers/blog-list-writer` (v3.0)  
**Validator**: `scripts/listicle_validator.py`  
...
### Category Winners Format (lintable lightly)
Must include 6–10 “winner blocks” each containing:
- Winner tool name
- `**Why it wins**:` line
- `**Choose**` + `**Avoid**` guidance (or equivalent)
...
- CTA#3: optional in Final Verdict (non-required)
...
Each alternative tool item must contain at least **one** Markdown table (a “mini side-by-side snapshot”) within its section.
