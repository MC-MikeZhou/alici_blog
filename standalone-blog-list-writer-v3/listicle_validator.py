#!/usr/bin/env python3
"""
Standalone Listicle Validator (Blog List Writer v3.0)

This is a self-contained copy of `scripts/listicle_validator.py` so you can lint
the 01/02/03 bundle outside the AliciBlog repo.

Usage:
  python3 listicle_validator.py --dir /path/to/output_dir
...
def validate_plan_pack(
    ...
    if not isinstance(selected, list) or not selected:
        errors.append(Finding(code="PLAN_SELECTED_TOOLS_MISSING", message="02-plan.json.selected_tools must be a non-empty list", details={}))
        selected = []

    for tool in selected:
        name = str((tool or {}).get("name") or "").strip()
        if not name:
            errors.append(Finding(code="PLAN_TOOL_NAME_MISSING", message="Each selected tool must have a name", details={"tool": tool}))
            continue
        if not str(tool.get("official_site_url") or "").strip():
            errors.append(Finding(code="PLAN_TOOL_OFFICIAL_URL_MISSING", message="Each selected tool must include official_site_url", details={"tool": name}))
        if not str(tool.get("pricing_url") or "").strip():
            errors.append(Finding(code="PLAN_TOOL_PRICING_URL_MISSING", message="Each selected tool must include pricing_url", details={"tool": name}))
    ...
    for tool in selected[:5]:
        ...
        if not third_party:
            warnings.append(Finding(code="EVIDENCE_TOP5_THIRD_PARTY_MISSING", message="Top 5 tool missing third-party evidence (warning only)", details={"tool": name}))
    ...
    if target_n is not None and selected and len(selected) != target_n:
        warnings.append(Finding(code="PLAN_LIST_SIZE_MISMATCH", message="selected_tools count differs from list_size_target (warning only)", details={"list_size_target": target_n, "selected_tools_count": len(selected)}))
...
def validate_assets_queue(assets_path: Path, errors: List[Finding]) -> Dict[str, Any]:
    ...
    if not isinstance(featured, dict) or not featured.get("alt") or not featured.get("size") or not featured.get("priority"):
        errors.append(Finding(code="ASSETS_FEATURED_IMAGE_MISSING", message="03-assets.json.featured_image must include alt, size, priority", details={}))
    diagrams = assets.get("diagrams") or []
    if not isinstance(diagrams, list) or len(diagrams) < 2:
        errors.append(Finding(code="ASSETS_DIAGRAMS_MIN", message="03-assets.json.diagrams must include at least 2 diagram requests", details={"count": len(diagrams) if isinstance(diagrams, list) else None}))
    return assets
...
def validate_article(
    ...
    h2_sections_titles = [s["title"] for s in h2_sections]
    h2_norm = [normalize_heading(t) for t in h2_sections_titles]
    ...
    if profile in ("standard", "prompt_workflow", "alternatives"):
        anchors = ["Quick Answer", "Key Takeaways", "Quick Comparison (Table)", "How We Picked & Tested"]
        if profile == "alternatives":
            anchors = ["Quick Answer", "Key Takeaways", "Ideal Solution Criteria", "Quick Comparison (Table)", "How We Picked & Tested"]
        tail = ["Use Case Matching (Table)", "How to Choose", "FAQ + Final Verdict"]
        if profile == "prompt_workflow":
            tail = [
                "Use Case Matching (Table)",
                "How to Prompt (Copy-Paste Templates)",
        ...
        if meth_i != -1 and use_i != -1 and use_i <= meth_i + 1:
            errors.append(Finding(code="TOOL_ITEMS_MISSING", message="Expected tool items between How We Picked & Tested and Use Case Matching (Table)", details={}))
        if meth_i != -1 and use_i != -1 and use_i > meth_i:
            between = h2_sections[meth_i + 1 : use_i]
            if not between:
                errors.append(Finding(code="TOOL_ITEMS_MISSING", message="No tool H2 items found in expected region", details={}))
            for sec in between:
                if not re.match(r"^\d+\.\s+", sec["title"]):
                    errors.append(Finding(code="TOOL_ITEM_HEADING_FORMAT", message="Tool item H2 must start with '{rank}.'", details={"heading": sec["title"]}))
            ...
            for sec in h2_sections:
                ...
                if re.match(r"^\d+\.\s+", t):
                    continue
                errors.append(Finding(code="ARTICLE_EXTRA_H2", message="Unexpected H2 heading (not fixed section, not tool item)", details={"heading": t}))

    if profile == "mega":
        fixed_top = ["Quick Answer", "Key Takeaways", "Quick Comparison (Table)", "How We Picked & Tested", "Category Winners"]
        last = -1
        for t in fixed_top:
            i = idx_of(t)
            if i != -1 and i < last:
        ...
        if winners_i != -1 and use_i != -1:
            ...
            if not groups:
                errors.append(Finding(code="MEGA_GROUPS_MISSING", message="Mega profile requires at least one group H2 between Category Winners and Use Case Matching (Table)", details={}))
            for g in groups:
                h3s = extract_h3_sections_within(g["content_lines"])
                if not h3s:
                    errors.append(Finding(code="MEGA_GROUP_NO_H3", message="Mega group must contain H3 tool cards", details={"group": g["title"]}))
                for h3 in h3s:
    ...
    if qa:
        ...
        if qa_words < 120 or qa_words > 180:
            errors.append(Finding(code="QUICK_ANSWER_WORD_COUNT", message="Quick Answer must be 120–180 words", details={"count": qa_words}))
    ...
    if final:
        ...
        if len(faq_items) < 6:
            errors.append(Finding(code="FAQ_MIN_COUNT", message="FAQ + Final Verdict must include at least 6 FAQ questions", details={"count": len(faq_items)}))
        for q, a in faq_items:
            s_count = count_sentences(a)
            if s_count < 2 or s_count > 4:
                errors.append(Finding(code="FAQ_ANSWER_SENTENCE_COUNT", message="Each FAQ answer must be 2–4 sentences", details={"question": q, "sentence_count": s_count}))
    ...
    if qc:
        ...
        if not m:
            errors.append(Finding(code="FRESHNESS_STATEMENT_MISSING", message="Quick Comparison section must include 'Verified as of YYYY-MM-DD'", details={}))
        elif freshness_date and m.group(1) != freshness_date:
            errors.append(Finding(code="FRESHNESS_DATE_MISMATCH", message="Freshness date in article must match 02-plan.json.meta.freshness_date", details={"article": m.group(1), "plan": freshness_date}))
        ...
        if meth_i != -1 and use_i != -1 and use_i > meth_i:
            for sec in h2_sections[meth_i + 1 : use_i]:
                if not find_tables(sec["content_lines"]):
                    errors.append(Finding(code="ALT_MINI_TABLE_MISSING", message="Each alternative tool item must include at least one mini table", details={"tool": sec["title"]}))
...
def _write_report(base_dir: Path, profile: str, errors: List[Finding], warnings: List[Finding], files: Dict[str, Any]) -> Dict[str, Any]:
    report = {
...
def main() -> int:
    ...
    if not article_path.exists():
        ...
        if alt.exists():
            ...
            errors.append(Finding(code="ARTICLE_MISSING", message="Missing required article file: 01-article-draft.md (or 01-article.md)", details={}))
    ...
    if profile not in ("standard", "prompt_workflow", "mega", "alternatives"):
        errors.append(Finding(code="PROFILE_INVALID", message="Invalid profile; must be standard|prompt_workflow|mega|alternatives", details={"profile": profile}))
